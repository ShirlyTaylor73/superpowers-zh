...
**核心原则：** wave 内并发派发实现者 + wave 间阻塞 gate + Wave FINAL 4 审 + 用户显式 ok = 高吞吐 + 高质量。每任务一个全新子智能体（隔离上下文）；commit 由 controller 在 wave 收口串行执行（避免并发 git 冲突）；同任务修复回路 ≤ 3 次后通过用户输入接口三选一（不维护 SKIP/BLOCKED 状态机）。
...
- 子智能体自然遵循 TDD
- 每个任务全新上下文（不会混淆）
- wave 内并发安全（4 条 pairwise 规则保证文件集互不相交）
- 子智能体可以提问（工作前和工作中都可以）

**与 Executing Plans 相比：**
- 同一会话（无交接）
- wave 内并发派发显著缩短墙钟时间（vs 串行）
- 审查检查点自动化（task-local spec + Wave FINAL 4 审）
...
- wave 内并发让独立任务并行推进（vs 旧版每任务串行）

**质量关卡：**
- 自审在交接前发现问题
- 任务级 spec-reviewer 审规格合规（task-local，可并发）
- Wave FINAL 4 审并发：F1 规格合规 / F2 代码质量 / F3 真实手测 / F4 范围保真，关注点互不重叠
- 文件集越界双层闸门（任务级 implementer 自报 + wave 收口终态）
- 用户显式 ok 是最后人机协同闸门
- 修复回路确保 reject 项被实际修复

**成本：**
- 子智能体调用：每任务 1 个 implementer + 1 个 spec-reviewer；全 plan 末 4 个 wave-final reviewer 并发派发
- 控制者需要更多准备工作（预先提取所有任务 + 解析 wave 图 + 加载时兜底校验）
- 修复回路按需触发（≤ 3 次后通过用户输入接口三选一）
- 但 wave 内并发让实际墙钟时间往往**比旧版串行更短**——成本上升，时间下降
