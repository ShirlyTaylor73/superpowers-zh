...
在列表中选中 `superpowers-zh` → 安装。插件市场方式会同时加载 skills 和 [`hooks/`](../plugins/superpowers-zh/hooks/)。
...
2. 创建 skills 符号链接：
   ...
   ln -s ~/.codex/superpowers-zh/plugins/superpowers-zh/skills ~/.agents/skills/superpowers
   ```
...
cmd /c mklink /J "$env:USERPROFILE\.agents\skills\superpowers" "$env:USERPROFILE\.codex\superpowers-zh\plugins\superpowers-zh\skills"
```
...
```
~/.agents/skills/superpowers/ → ~/.codex/superpowers-zh/plugins/superpowers-zh/skills/
```
